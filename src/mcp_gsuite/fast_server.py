...
from .calendar_tools import (create_calendar_event, delete_calendar_event,
                             list_calendar_events, list_calendars)
from .drive_tools import download_drive_file, get_drive_file, list_drive_files
from .gmail_tools import (bulk_get_gmail_emails, bulk_save_gmail_attachments,
                          create_gmail_draft, create_gmail_reply,
                          delete_gmail_draft, get_email_details,
                          get_gmail_attachment, get_gmail_labels,
                          query_gmail_emails)
    ...
    if _account_info_cache is None:
        try:
            ...
            if not _account_info_cache:
                logger.warning(
                    "No accounts found in accounts file. User ID description will be generic."
                )
                return "The EMAIL of the Google account to use."
        except Exception as e:
            logger.error(f"Failed to load account info for user ID description: {e}")
            return (
                "The EMAIL of the Google account to use (Error loading account list)."
            )
...
mcp.tool(description="Create a reply to an existing Gmail email message.")(
    create_gmail_reply
)
...
mcp.tool(
    description="List files in the user's Google Drive with optional filtering by search query."
)(list_drive_files)

mcp.tool(description="Get metadata for a specific Google Drive file by its ID.")(
    get_drive_file
)

mcp.tool(description="Download the content of a Google Drive file by its ID.")(
    download_drive_file
)
